function chidx = jReceptiveFieldIdx(nwb, p)
    ...
    fprintf("-> %d Probes detected in %s\n-->This function will process only probe no.%d (%s) \n", numProbes, identifier_, p, ProbeAreas{p}{1});
    ...
    for chan = 1:size(resp, 1)
        ...
        idxtemp = chanresp == max(chanresp);
        chidx(chan, 1:3) = uniconds(idxtemp, 1:3);
        chidx(chan, 4) = max(chanresp);
