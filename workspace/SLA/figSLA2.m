...
for iN = 1:size(gmatrix1, 1)
    ...
    % Get spectra peak
    thetaP = 2.0 + fs(temp_relpow(fs < 7.5 & fs > 2.0) == max(temp_relpow(fs < 7.5 & fs > 2.0)));
    thetaPeak(1, iN) = thetaP(1);
    alphaP = 8.0 + fs(temp_relpow(fs < 12.0 & fs > 8.0) == max(temp_relpow(fs < 12.0 & fs > 8.0)));
    alphaPeak(1, iN) = alphaP(1);
    betaP = 12.0 + fs(temp_relpow(fs < 30.0 & fs > 12.0) == max(temp_relpow(fs < 30.0 & fs > 12.0)));
    betaPeak(1, iN) = betaP(1);
    gammaP = 35.0 + fs(temp_relpow(fs > 35.0) == max(temp_relpow(fs > 35.0)));
...
g5_oxcorr = find(abs(xr2) > 0.12 & xAFR > 1.5*bAFR);
