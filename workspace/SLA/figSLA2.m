...
thetaPeak = zeros(2, size(gmatrix1, 1));
alphaPeak = zeros(2, size(gmatrix1, 1));
...
for iN = 1:size(gmatrix1, 1)
    ...
    % Get signal
    temp_sigxc = find(squeeze(sspkDataCleanTrials{fileIDs(iN), rxrr}(:, infileIDs(iN))) == 1);
    temp_sigx = squeeze(sspkData{fileIDs(iN), rxrr}(temp_sigxc, infileIDs(iN), :));
...
xt = ones(1, 4750)*-1;
xt(1, 1531:2200) = 1;
xr = squeeze(gmatrixFR1(10, :, :))';
xr2 = corr(xt', xr);

xt = ones(1, 4750)*-1;
xt(1, 2561:3200) = 1;
xr = squeeze(gmatrixFR1(11, :, :))';
xr3 = corr(xt', xr);

xt = ones(1, 4750)*-1;
xt(1, 3591:4200) = 1;
xr = squeeze(gmatrixFR1(12, :, :))';
xr4 = corr(xt', xr);
...
for iA = 1:11
    ...
    gvec1(1, iA) = length(generalIDs);
    % gvec1(1, iA) = sum(ismember(generalIDs, g1_fprime));
    gvec1(2, iA) = sum(ismember(generalIDs, g2_sprime));
    % gvec1(2, iA) = sum(ismember(generalIDs, g1_ifprime));
    gvec1(3, iA) = sum(ismember(generalIDs, g2_invsprime));
    gvec1(4, iA) = sum(ismember(generalIDs, g4_ssprime));
    gvec1(5, iA) = sum(ismember(generalIDs, g1_oxprime));
    gvec1(6, iA) = sum(ismember(generalIDs, g5_oxcorr));
...
subplot(2, 3, 1);
piechart(gvec1(1, :), areaList_temp, "LabelStyle", "namedata", "ColorOrder", color_t);
title("iFR responsive N=" + num2str(sum(gvec1(1, :))))

areaList_temp = areaList;
pct1 = 100*gvec1(2, :)./gvec1(1, :);
for iA = 1:11
    areaList_temp(iA) = areaList_temp(iA) + "(" + num2str(pct1(iA), 2) + "%)";
end

subplot(2, 3, 2);
piechart(gvec1(2, :), areaList_temp, "LabelStyle", "namedata", "ColorOrder", color_t);
title("Excited by stimulus N=" + num2str(sum(gvec1(2, :))));
% title("nl N=" + num2str(sum(gvec1(2, :))));

areaList_temp = areaList;
pct1 = 100*gvec1(3, :)./gvec1(1, :);
for iA = 1:11
    areaList_temp(iA) = areaList_temp(iA) + "(" + num2str(pct1(iA), 2) + "%)";
end

subplot(2, 3, 3);
piechart(gvec1(3, :), areaList_temp, "LabelStyle", "namedata", "ColorOrder", color_t);
title("Inhibited by stimulus N=" + num2str(sum(gvec1(3, :))));

areaList_temp = areaList;
pct1 = 100*gvec1(4, :)./gvec1(1, :);
for iA = 1:11
    areaList_temp(iA) = areaList_temp(iA) + "(" + num2str(pct1(iA), 2) + "%)";
end

subplot(2, 3, 4);
piechart(gvec1(4, :), areaList_temp, "LabelStyle", "namedata", "ColorOrder", color_t);
title("Stimulus informative (PEV > 20%; High) N=" + num2str(sum(gvec1(4, :))));

areaList_temp = areaList;
pct1 = 100*gvec1(5, :)./gvec1(1, :);
for iA = 1:11
    areaList_temp(iA) = areaList_temp(iA) + "(" + num2str(pct1(iA), 2) + "%)";
end

subplot(2, 3, 5);
piechart(gvec1(5, :), areaList_temp, "LabelStyle", "namedata", "ColorOrder", color_t);
title("Any omission informative (PEV > 1%) N=" + num2str(sum(gvec1(5, :))));

areaList_temp = areaList;
pct1 = 100*gvec1(6, :)./gvec1(1, :);
for iA = 1:11
    areaList_temp(iA) = areaList_temp(iA) + "(" + num2str(pct1(iA), 2) + "%)";
end

subplot(2, 3, 6);
piechart(gvec1(6, :), areaList_temp, "LabelStyle", "namedata", "ColorOrder", color_t);
title("Omission-specific correlated activity (r > .1) N=" + num2str(sum(gvec1(6, :))));
...
function [X_ellipse, Y_ellipse] = fitConfidenceEllipse(X_data, Y_data, N, CI, mode, rect)
    ...
    if rect
        ...
        X_ellipse(indxtemp1) = 0.5 + 0.5*exp(X_ellipse(indxtemp1) - 1);
        indxtemp1 = find(Y_ellipse < 1);
        Y_ellipse(indxtemp1) = 0.5 + 0.5*exp(Y_ellipse(indxtemp1) - 1);
